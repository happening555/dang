...
// Helper to fetch Instagram following count
async function fetchInstagramFollowing(username) {
    const req = await fetch("https://instagram.com/"+username+'/', {
        "credentials": "omit",
        "headers": {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:130.0) Gecko/20100101 Firefox/130.0",
    ...
    const res = await req.text();
    // Try to extract following count from meta tag
    const match = res.match(/([\d,.]+) Followers, ([\d,.]+) Following/);
    if (match) {
        return {
            followers: match[1],
            following: match[2]
    ...
    if (message.content.startsWith('!giveaccess')) {
        ...
        if (args.length < 2 || !args[1]) {
            const embed = new EmbedBuilder()
                .setAuthor({ name: `Requested by @${message.author.username}` })
                .setTitle('❌ Missing Username')
                .setDescription('You need to specify a username to fetch following count.\n\n**Usage:** `!fetch <username>`')
                .setColor(0xFF0000)
                .setThumbnail(message.author.displayAvatarURL())
                .setFooter({ text: 'Please try again', iconURL: client.user.displayAvatarURL() });
            await message.channel.send({ embeds: [embed] });
        ...
        const username = args[1];
        try {
            const result = await fetchInstagramFollowing(username);
            if (result) {
                const embed = new EmbedBuilder()
                    .setAuthor({ name: `Requested by @${message.author.username}` })
                    .setTitle(`Instagram Following Lookup | @${username}`)
                    .setDescription(`**Followers:** ${result.followers}\n**Following:** ${result.following}`)
                    .setColor(0x0095F6)
                    .setThumbnail(`https://instagram.com/${username}/avatar/`)
                    .setFooter({ text: 'Instagram Lookup', iconURL: client.user.displayAvatarURL() });
                await message.channel.send({ embeds: [embed] });
            } else {
                const embed = new EmbedBuilder()
                    .setAuthor({ name: `Requested by @${message.author.username}` })
                    .setTitle('❌ Could Not Fetch')
                    .setDescription(`Could not fetch following info for **@${username}**. The account may be private, banned, or does not exist.`)
                    .setColor(0xFF0000)
                    .setThumbnail(message.author.displayAvatarURL())
                    .setFooter({ text: 'Instagram Lookup', iconURL: client.user.displayAvatarURL() });
                await message.channel.send({ embeds: [embed] });
            ...
            const embed = new EmbedBuilder()
                .setAuthor({ name: `Requested by @${message.author.username}` })
                .setTitle('❌ Error')
                .setDescription(`An error occurred while fetching info for **@${username}**.`)
                .setColor(0xFF0000)
                .setFooter({ text: 'Instagram Lookup', iconURL: client.user.displayAvatarURL() });
            await message.channel.send({ embeds: [embed] });
        }
        ...
        const embed = new EmbedBuilder()
            ...
            **!fetch <username>** - Looks up an Instagram account's followers and following count.
